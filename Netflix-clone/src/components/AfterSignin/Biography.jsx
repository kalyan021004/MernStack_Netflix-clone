...
const API_URL ="https://mernstack-netflix-clone-cpvy.onrender.com";
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/titles/genre/biography`, {
          withCredentials: true
        });
        setMovies(res.data);
      } catch (err) {
        console.error("Failed to fetch Biography movies:", err);
        setError("Failed to load Biography movies.");
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="text-white">Biography Movies</h2>
      ...
      {!loading && !error && movies.length === 0 && (
        <p className="text-light">No Biography movies found.</p>
      )}
